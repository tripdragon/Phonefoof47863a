...
export class DirectionArrow{
	...
	build(){
		...
		this.ff.visualsObject3D.add(this.arrowDirHelper);
	}


	reset(){
		this.movingAveragePointV.set(0,0,0);
		this.arrowDirOriginV.set(0,0,0);
		this.arrowDirV.set(0,0,0);
	}
	...
	refresh(){
		...
		if(planePoints.length > this.waitLimToUpdateAveDir){
			...
			this.arrowDirOriginV.copy(this.ff.getPointDown());
	...
	*/
	...
	updateDirectionCheck(ev){
		...
		this.getAveragePointFromHits(planePoints, this.movingAveragePointV);
