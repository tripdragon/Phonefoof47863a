...
test("release slerps a positive drag to the next quarter turn and refishes groups", () => {
  const { controller } = makeController({
    hasActivePointers: false,
    shouldSkipTouchUp: false,
  });
  ...
  try {
    ...
    const appliedAngles = [];
    let refishCount = 0;
    controller.state = "activeTumbling";
    controller.lastTumbleDelta = 0.6;
    controller.engines.plucker.plucked.group = group;
    controller.ff.cube = {
      spinGroup({ group: spunGroup, deltaAngle }) {
        assert.equal(spunGroup, group);
        appliedAngles.push(deltaAngle);
      },
      refishGroups() { refishCount += 1; },
    };

    controller.onPointerUp({ pointerId: 1 });
    assert.equal(controller.state, "snapping");

    callbacks.shift()(0);
    callbacks.shift()(125);
    callbacks.shift()(250);

    const snappedAngle = 0.6 + appliedAngles.reduce((sum, angle) => sum + angle, 0);
    assert.ok(Math.abs(snappedAngle - Math.PI / 2) < 1e-12);
    assert.equal(refishCount, 1);
...
test("release follows a negative drag to the next negative quarter turn", () => {
  const { controller } = makeController({});
  const callbacks = [];
  const originalRequestAnimationFrame = globalThis.requestAnimationFrame;
  globalThis.requestAnimationFrame = callback => callbacks.push(callback);
  ...
  try {
    const group = { axis: new Vector3(1, 0, 0) };
    const appliedAngles = [];
    controller.lastTumbleDelta = -0.4;
    controller.engines.plucker.plucked.group = group;
    controller.ff.cube = {
      spinGroup({ deltaAngle }) { appliedAngles.push(deltaAngle); },
      refishGroups() {},
    };

    assert.equal(controller.beginReleaseSnap(), true);
    callbacks.shift()(0);
    callbacks.shift()(250);

    const snappedAngle = -0.4 + appliedAngles.reduce((sum, angle) => sum + angle, 0);
    assert.ok(Math.abs(snappedAngle + Math.PI / 2) < 1e-12);
  } finally {
    globalThis.requestAnimationFrame = originalRequestAnimationFrame;
  }
